...
export function Header({ animatedScrollY }: Props) {
  ...
  const containerStylesAnimated = useAnimatedStyle(() => {
    return {
      ...
      borderBottomWidth: interpolate(animatedScrollY.value, [0, 380], [0, 1], Extrapolate.CLAMP),
      borderBottomColor: interpolateColor(animatedScrollY.value, [0, 380], ['transparent', THEME.COLORS.BASE_GRAY_800])
    };
  });
  ...
  const locationLabelStylesAnimated = useAnimatedStyle(() => {
    return {
      color: interpolateColor(
        animatedScrollY.value,
        [0, 300],
        [THEME.COLORS.BASE_GRAY_900, THEME.COLORS.BASE_GRAY_100]
      ),
    };
  });
    ...
    <Animated.View style={[styles.container, containerStylesAnimated]}>
      <View style={styles.content}>
        <View style={styles.locationContainer}>
          ...
          <Animated.Text style={[styles.locationLabel, locationLabelStylesAnimated]}>
